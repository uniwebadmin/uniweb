    ...
    <main class="portal-main flex-1 lg:ml-64 flex flex-col min-w-0 w-full">
        <header class="portal-top-bar bg-dark-900 border-b border-gray-800 px-3 sm:px-6 py-3 sticky top-0 z-20 flex items-center gap-2">
            ...
            <?php
                ...
                $openSupportHdr = 0;
                try {
                    $openSupportHdr = (int)(getDB()->query("SELECT COUNT(*) FROM support_tickets WHERE status IN ('open','in_progress')")->fetchColumn() ?: 0);
                } catch (Throwable $e) {
                    $openSupportHdr = 0;
                }
            ?>
    ...
    <main class="portal-main flex-1 lg:ml-64 flex flex-col min-w-0 w-full">
        <header class="portal-top-bar bg-dark-900 border-b border-gray-800 px-3 sm:px-6 py-3 sticky top-0 z-20 flex items-center gap-2">
            ...
            <div class="portal-header-tools flex items-center gap-1 sm:gap-2 shrink-0 flex-nowrap">
                ...
                <?php require __DIR__ . '/includes/global_search_ui.php'; ?>
                <?php
                $openSupportStaffHdr = 0;
                try {
                    $openSupportStaffHdr = (int)(getDB()->query("SELECT COUNT(*) FROM support_tickets WHERE status IN ('open','in_progress')")->fetchColumn() ?: 0);
                } catch (Throwable $e) {
                    $openSupportStaffHdr = 0;
                }
                if ($openSupportStaffHdr > 0 && function_exists('staffCanAccess') && staffCanAccess('admin_support.php')):
                ?>
