...
<div class="glass rounded-xl overflow-hidden mb-8 border border-amber-500/30 min-w-0">
    <div class="px-4 sm:px-6 py-4 border-b border-gray-800"><h2 class="font-semibold">Independent checker queue</h2><p class="text-xs text-gray-500 mt-1">The maker who requested an action cannot approve it. CSRF + step-up on approve.</p></div>
    ...
    <div class="px-4 sm:px-6 py-4 border-b border-gray-800 flex flex-col sm:flex-row flex-wrap items-stretch sm:items-center justify-between gap-3">
        ...
        <div class="flex flex-col sm:flex-row gap-2 w-full sm:w-auto">
            <form method="post" class="flex flex-col sm:flex-row gap-2 min-w-0 flex-1"><input type="hidden" name="csrf_token" value="<?= csrfToken() ?>"><input type="hidden" name="action" value="approve_request"><input type="hidden" name="id" value="<?= (int)$request['id'] ?>"><input name="reason" required maxlength="500" placeholder="Checker reason" class="text-xs bg-gray-900 border border-gray-700 rounded-lg px-2 py-1.5 w-full"><button class="text-xs bg-emerald-600 text-white px-3 py-2 rounded-lg w-full sm:w-auto">Approve</button></form>
            <form method="post" class="flex flex-col sm:flex-row gap-2 min-w-0 flex-1"><input type="hidden" name="csrf_token" value="<?= csrfToken() ?>"><input type="hidden" name="action" value="reject_request"><input type="hidden" name="id" value="<?= (int)$request['id'] ?>"><input name="reason" required maxlength="500" placeholder="Rejection reason" class="text-xs bg-gray-900 border border-gray-700 rounded-lg px-2 py-1.5 w-full"><button class="text-xs bg-red-600/20 text-red-400 px-3 py-2 rounded-lg w-full sm:w-auto">Reject</button></form>
        </div>
    </div>
    <?php endforeach; ?>
</div>
...
<div class="glass rounded-xl overflow-hidden mb-8 border border-violet-500/30 min-w-0">
    ...
    <div class="px-4 sm:px-6 py-4 border-b border-gray-800 flex flex-col sm:flex-row flex-wrap items-stretch sm:items-center justify-between gap-3">
        ...
        <div class="flex flex-col sm:flex-row gap-2 flex-wrap w-full sm:w-auto">
            ...
            <form method="post" class="flex flex-col sm:flex-row gap-2 items-stretch sm:items-center min-w-0 flex-1"><input type="hidden" name="csrf_token" value="<?= csrfToken() ?>"><input type="hidden" name="action" value="reject_video"><input type="hidden" name="id" value="<?= (int)$videoRow['id'] ?>"><input name="reason" required maxlength="500" placeholder="Rejection reason (shown to merchant)" class="text-xs bg-gray-900 border border-gray-700 rounded-lg px-2 py-1.5 w-full min-w-0"><button class="text-xs bg-red-600/20 text-red-400 px-3 py-2 rounded-lg w-full sm:w-auto shrink-0">Reject video</button></form>
        </div>
    </div>
    <?php endforeach; ?>
</div>
        ...
        <div class="px-4 sm:px-6 py-4 border-b border-gray-800">
            <div class="flex justify-between items-start mb-2 gap-2">
                <div class="min-w-0">
                    ...
                    <p class="text-xs text-gray-500 capitalize break-words"><?= e(entityTypeLabel($doc['business_entity_type'] ?? '')) ?> — <?= str_replace('_', ' ', $doc['doc_type']) ?></p>
                </div>
            ...
            <div class="flex gap-2 mt-2 flex-wrap">
                ...
                <form method="post" class="flex flex-col sm:flex-row gap-2 w-full sm:w-auto min-w-0"><input type="hidden" name="csrf_token" value="<?= csrfToken() ?>"><input type="hidden" name="action" value="reject_doc"><input type="hidden" name="id" value="<?= (int)$doc['id'] ?>"><input name="reason" required maxlength="500" placeholder="Clarification reason" class="text-xs bg-gray-900 border border-gray-700 rounded-lg px-2 py-1.5 w-full"><button class="text-xs bg-red-600/20 text-red-400 px-3 py-1.5 rounded-lg w-full sm:w-auto">Reject</button></form>
                <?php endif; ?>
            </div>
        </div>
