...
$assert(str_contains($kycFlow, 'function advanceMerchantForwardAfterVerify') && str_contains($kycFlow, 'advanceMerchantForwardAfterVerify($merchantId, kycForwardSourceFromVerifyActor($source))'), 'kyc_verify_kicks_forward_to_staged');
...
$assert(str_contains($colVaPage, 'getMerchantVirtualAccounts($merchantId)') && !str_contains($colVaPage, "collectionMode === 'axis_va' || getMerchantPrimaryVaNumber"), 'va_merchant_list_always_loads');
$assert(str_contains($colVaPage, 'Bank') && str_contains((string)file_get_contents($root . '/dashboard.php'), 'getMerchantVirtualAccounts'), 'va_merchant_bank_column_and_dashboard');
$assert(str_contains($vaLib, 'function findVirtualAccountByNumber') && str_contains((string)file_get_contents($root . '/includes/rbl.php'), 'function rblVaSerialsForCreate'), 'va_rbl_unique_serial_and_duplicate_reuse');
...
$assert(str_contains((string)file_get_contents($root . '/includes/pg_webhooks.php'), 'function pgWebhookBindCollectContext') && str_contains((string)file_get_contents($root . '/includes/pg_webhooks.php'), 'function pgWebhookLookupCollectOrder'), 'already_live_webhook_binds_collect_context');
$assert(str_contains((string)file_get_contents($root . '/includes/webhook_secret_rotation.php'), 'resolveCollectPartnerCredentials') && str_contains((string)file_get_contents($root . '/includes/webhook_secret_rotation.php'), "!== 'merchant_link'"), 'already_live_webhook_secrets_from_link');
...
$assert(str_contains((string)file_get_contents($root . '/includes/webhook_reliability.php'), 'function webhookRetryBindCollectContext'), 'already_live_webhook_retry_binds_collect');
$assert(str_contains((string)file_get_contents($root . '/includes/refund_webhooks.php'), 'setCollectCredentialContext') && str_contains((string)file_get_contents($root . '/includes/refund_webhooks.php'), 'txn_merchant_id'), 'already_live_refund_webhook_collect_keys');
$assert(str_contains((string)file_get_contents($root . '/payment_methods.php'), 'razorpay_webhook.php') && str_contains((string)file_get_contents($root . '/payment_methods.php'), 'already_live_webhook_secret'), 'already_live_merchant_webhook_url_copy');
...
$assert(str_contains($fwdQSrc, "'adapter' => 'sandbox_stub'") && str_contains($fwdQSrc, 'Sandbox test ACK — not live network'), 'kyc_sandbox_stub_not_live_success');
$assert(str_contains($fwdQSrc, "status IN ('staged')") && !str_contains($fwdQSrc, "status IN ('staged','success')"), 'kyc_needs_action_no_fake_success');
$assert(str_contains($fwdUi, 'name="event_type"') && str_contains($fwdUi, 'name="note"') && str_contains($fwdUi, 'Log need docs'), 'kyc_log_need_docs_form');
$assert(str_contains((string)file_get_contents($root . '/kyc.php'), 'Enabled (docs only)') && str_contains((string)file_get_contents($root . '/kyc.php'), 'honest_note'), 'kyc_coverage_honest_note_and_docs_only');
$assert(str_contains((string)file_get_contents($root . '/includes/kyc_workflow.php'), 'function kycForwardSourceFromVerifyActor') && str_contains((string)file_get_contents($root . '/includes/kyc_workflow.php'), "return 'kyc_verify'"), 'kyc_auto_verify_tags_kyc_verify');
