# Hostinger File Manager ΓÇö clean after every Git pull
...
1. Hostinger ΓåÆ **Files ΓåÆ Backups**  
2. phpMyAdmin ΓåÆ **Export** SQL  
3. File Manager ΓåÆ `public_html` ΓåÆ **delete these folders if present:**

| Delete | Why |
|--------|-----|
| `_inbox/` | Agent notes, photos, owner chat ΓÇö **not the website**. Had a leaked env sample; remove forever. |
| `dev_local/` | Laptop DB tools / PDF scratch |
| `tests/` | Smoke scripts ΓÇö CLI only |
| `scripts/` | One-off helpers |
| `training/` | Outline notes |
| `docs/` | Internal markdown |
| `.cursor/` `.devin/` `.github/` | Agent / CI ΓÇö not needed on web |
...
- **Apply pending migrations** (Gateway Settings) ΓÇö never DROP DATABASE  
...
## From Latest Status Audit PDF (backup 15 Aug) ΓÇö also do
...
4. **CR-04/05:** Partner Detail ΓåÆ Commercial save once + paste Test keys; SMTP.
5. **CR-06:** Apply pending migrations ΓåÆ `ok: true`.

## Next (Owner)

Partner keys ΓåÆ Partner Registry ΓåÆ Test collect ΓåÆ then advertise for new merchants.
