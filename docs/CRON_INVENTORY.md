# Cron Inventory ΓÇö Hostinger Setup
...
## Required on Hostinger ΓÇö 1 job only

| Schedule | What to paste | Already includes |
|----------|----------------|------------------|
| Every 10 min | Gateway Settings ΓåÆ **Show full Hostinger command** (wget or URL) | Watchdog, auto KYC, settlements, recurring mandates, partner forward, payout queue, recon mark, morning ops |
...
Copy the real command from Admin ΓåÆ Gateway Settings (logged-in reveal). Do not invent a key.

## Not a cron

| Task | Where |
|------|--------|
| Database updates (migrations) | Gateway Settings ΓåÆ **Apply pending migrations** (one click after deploy) |
...
Separate `cron_auto_kyc.php` / `cron_settlements.php` / `cron_mandates.php` jobs are **not required** ΓÇö the 10-minute job already runs them. They still accept the same UniWeb watchdog key if you add extras.

## Browser ΓÇ£ForbiddenΓÇ¥ text (expected)
...
- Humans check health in **Admin ΓåÆ Link Watchdog** and **Platform Status + Cron Jobs**.
- Cron keys stay in Gateway Settings; never put them in menus or public pages.

## After saving the Hostinger job

1. Gateway Settings ΓåÆ **Test cron now**
2. Wait 10ΓÇô15 min
3. Admin ΓåÆ Platform Status ΓÇö Auto Audit should show last run, not NEVER
