# Block A ΓÇö Hostinger & database cleanup

**Status (2026-08-15):** Code/UI safety fixes shipped. **Hostinger file deletes wait on Owner backup (LIVE-03).**  
**Rule:** if unsure ΓåÆ leave it, mark REVIEW. Never hard-delete money rows. Never `DROP DATABASE`.

**LIVE-03 order:** Files Backups + full SQL export ΓåÆ then delete junk only (lists below). See also `_inbox/chat/LIVE_03_backup_before_cleanup.txt` and **`HOSTINGER_CLEAN_NOW.md`** (why Git pull brings laptop folders).

Related: `CLEANUP_SENSITIVE_CLICKABLE_AUDIT.md` ┬╖ Block D order = backup ΓåÆ B ΓåÆ C ΓåÆ **A** ΓåÆ E.  
**LIVE-02 (after Git pull):** Apply pending migrations (062/063) ΓåÆ Encrypt PII Backfill if needed ΓÇö never DROP DATABASE.

---

## A-04 ┬╖ Safety checklist (do this first)

1. Hostinger **Files ΓåÆ Backups** (full website).  
2. phpMyAdmin ΓåÆ **Export** full SQL dump (save outside public_html).  
...
| `tests/` | CLI smoke only ΓÇö not the live site |
| `scripts/` | Overnight / one-time helpers |
| `dev_local/` | Laptop MariaDB tools |
| `training/`, `docs/`, `_inbox/`, `archive/` | Notes / PDFs / screenshots ΓÇö not product |
...
`ping.php` (health probe), `mobile.php` (phone hub), `config.dev.php` (template ΓÇö live uses `config.php`). Do not restore deleted NBFC / customer wallet pages.
...
- FTP deploy workflow excludes those folders (Hostinger **Git** pull may still copy them ΓÇö delete on server after backup if they appear).

**Deletions log (repo):** none from live yet ΓÇö Owner action pending.
...
### Soft UPDATE examples (only after Owner OK ΓÇö edit IDs)
...
| Issue | Fix |
|-------|-----|
| Onboarding ΓÇ£Γé╣1 test paymentΓÇ¥ ΓåÆ missing `merchant_launch_test.php` | Now ΓåÆ `merchant_payment_pack.php` |
| Admin demo table ΓåÆ missing `platform_demo.php` | Now ΓåÆ `demo.php` |
...
No empty ΓÇ£Coming soonΓÇ¥ blocks found on merchant/admin dashboards in this pass.
