# UniWeb ΓÇö Cleanup ┬╖ Sensitive data ┬╖ Clickable links (audit addendum)
...
## BLOCK B ΓÇö Sensitive data encryption & display rules
...
## BLOCK C ΓÇö Clickable links & dead ends
...
## BLOCK D ΓÇö Implementation order (keep system stable)
...
## BLOCK E ΓÇö Owner verification checklist
...
- **P0-04 live `config.php`** stays parked until this cleanup addendum work is done ΓÇö then remind Owner.  
- Do not DROP production DB. Do not unhide NBFC / customer PPI.
- **2026-08-15 progress:** Block B complete locally ΓÇö widen cipher columns (migration 062 + `ensureSensitivePiiColumnWidths`); `encryptSensitive`/`decryptSensitive`/`sensitiveUiPlain`/`decryptMerchantPiiFields`; merchant+admin+invoice decrypt on read; address encrypted on write; partner outbound decrypts before JSON; Encrypt PII backfill includes address; login email/phone stay plaintext. **Block C** done (clickable cards/names). **Block A** ΓÇö see `BLOCK_A_CLEANUP.md`: dead checklist/demo links fixed; `.htaccess` + FTP exclude junk dirs; **Hostinger deletes wait on Owner backup**; DB cleanup = Owner-reviewed SELECT/UPDATE only.
- **Owner still must (Block D-1):** Hostinger Files ΓåÆ Backups + phpMyAdmin SQL export before any Hostinger file delete (Block A).
