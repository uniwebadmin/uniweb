...
<div class="container mx-auto px-4 py-6">
    ...
    <form method="GET" class="glass rounded-xl p-4 mb-6 border border-gray-800 grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3 items-end">
        ...
        <div><label class="text-[10px] text-gray-600 uppercase block mb-1">Partner</label><select name="partner" class="input-field text-sm w-full"><?php foreach ($partnerChoices as $k=>$v): ?><option value="<?= e($k) ?>" <?= $partnerFilter===$k?'selected':'' ?>><?= e($v) ?></option><?php endforeach; ?></select></div>
        <div><label class="text-[10px] text-gray-600 uppercase block mb-1">Merchant</label><select name="merchant_id" class="input-field text-sm w-full"><option value="0">All</option><?php foreach ($merchants as $m): ?><option value="<?= (int)$m['id'] ?>" <?= $merchantFilter===(int)$m['id']?'selected':'' ?>><?= e($m['business_name']) ?></option><?php endforeach; ?></select></div>
        <div class="flex gap-2 items-end">
        <button class="btn-primary px-4 py-2.5 text-sm whitespace-nowrap">Filter</button>
        <a href="admin_payment_links.php?export=csv&csrf_token=<?= rawurlencode(csrfToken()) ?><?= ($q ? '&q=' . rawurlencode($q) : '') . ($statusFilter !== 'all' ? '&status=' . $statusFilter : '') . ($modeFilter !== 'all' ? '&mode=' . $modeFilter : '') . ($partnerFilter !== 'all' ? '&partner=' . $partnerFilter : '') . ($merchantFilter ? '&merchant_id=' . $merchantFilter : '') ?>" class="glass px-3 py-2.5 rounded-lg text-xs text-brand-400 hover:text-brand-300 whitespace-nowrap">Export CSV</a>
        </div>
    </form>
