...
$listSql = 'SELECT d.*, m.business_name, m.id AS merchant_row_id, t.txn_id, t.amount, t.partner_key AS txn_partner_key, COALESCE(d.partner_key, t.partner_key) AS dispute_partner_key FROM disputes d JOIN merchants m ON d.merchant_id=m.id JOIN transactions t ON t.id=d.transaction_id';
$listWhere = [];
$listParams = [];
if ($filterMerchantId > 0) {
...
if ($highlightDisputeId !== '' && function_exists('wiringAdminDisputesEnsureHighlightedRow')) {
    ...
    if (!$exact) {
        try {
            $exSt = $db->prepare(
                'SELECT d.*, m.business_name, m.id AS merchant_row_id, t.txn_id, t.amount, t.partner_key AS txn_partner_key, COALESCE(d.partner_key, t.partner_key) AS dispute_partner_key
                 FROM disputes d
    ...
    <table class="min-w-[720px] w-full text-sm">
        ...
        <tbody class="divide-y divide-gray-800">
            ...
            <?php else: foreach ($disputes as $d):
                ...
                $rowPartner = function_exists('resolveDisputePartnerKey') ? resolveDisputePartnerKey($d, $d) : '';
            ...
            <tr id="dispute-<?= e($d['dispute_id']) ?>" class="<?= $highlightDisputeId !== '' && strcasecmp((string)$d['dispute_id'], $highlightDisputeId) === 0 ? 'bg-sky-500/10 ring-1 ring-sky-500/30' : '' ?>">
                ...
                <td class="px-4 sm:px-5 py-3 text-xs text-sky-300"><?= e($rowPartnerLabel) ?></td>
