...
2) **THE EXPECTATION:** Merchant → dashboard.php; staff/admin → staff/admin dashboard; clear flash. Never bare white exit for logged-in users.
...
1) **THE PROBLEM:** Footer correctly links pci_dss.php; /pci.php does not exist → 404 for old bookmarks.
...
3) **THE SOLUTION / ACTION:** Remove video_kyc.php from sidebar; link KYC → video section. Keep alias files.
...
1) **THE PROBLEM:** Cron scripts exit Forbidden when hit without key — looks like white text if opened in browser.
...
1) **THE PROBLEM:** None critical — Payment Links, QR, Instant UPI QR, Payment Methods map to collect.
...
3) **THE SOLUTION / ACTION:** Keep page; prefer nav label Settlement Balance; solutions.php copy Settlements & wallet → Settlements & balance.
...
## 3 · Panels A–Z (Admin, Merchant, Sub-merchant, Staff, Team, Customer)

### PNL-A01 · Admin panel — keys only in Partner Registry

1) **THE PROBLEM:** Historical dual key UIs confuse ops (platform paste vs Registry).

2) **THE EXPECTATION:** Live secrets only on Partner Detail → Keys; Platform Settings = SMTP/cron/templates.
...
### PNL-M01 · Merchant panel — Fixed and Open payment links
...
1) **THE PROBLEM:** None — intentional exclusion from staff nav.
...
2) **THE EXPECTATION:** Allowed roles ⊆ defined roles.
...
1) **THE PROBLEM:** No global search; no wallet — some owners may ask for both.
...
### DUP-08 · cust.php and payer.php both → customer login
...
1) **THE PROBLEM:** Owner asked A–Z everything searchable.
...
Peers: Juspay, Cashfree, Razorpay, Stripe, Worldline, PayU, Decentro. Reference only — do not clone catalogues before Phase 0–2 live green.
...
1) **THE PROBLEM:** Merchants judge create link → pay → settle and print QR → scan.
...
What a bank/fintech may demand. Implement a row only if Owner names a deal — after Phase 0–2. Do not put Buy white-label on homepage.
...
1) **THE PROBLEM:** Enterprise wants clear MID ↔ partner rail matrix.
...
2) **THE EXPECTATION:** Pull → hard refresh → smoke.
